...
const updateUserById = async (userId, updateBody, files) => {
  ...
  const { firstName, lastName } = updateBody;
...
//user subscribe status update

const subscribeEndingUserStatusUpdate = async () => {
  const currentDate = new Date();
 await User.updateMany(
    { isSubscribe: true, subscriptionEnd: { $lt: currentDate } },
    {
      $set: {
        isSubscribe: false,
...
module.exports = {
  ...
  deleteUserById,
  isUpdateUser,
  subscribeEndingUserStatusUpdate
};
